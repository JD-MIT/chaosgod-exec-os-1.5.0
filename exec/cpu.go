...
func (ce *cpuExecutor) Exec(uid string, ctx context.Context, model *spec.ExpModel) *spec.Response {
	...
	if absoluteStr != "" {
		...
		if err != nil {
			util.Errorf(uid, util.GetRunFuncName(), fmt.Sprintf("`%s`: absolute is illegal, absolute value must be bool type", absoluteStr))
			return spec.ResponseFailWithFlags(spec.ParameterIllegal, "absolute", absoluteStr, "it must be a boolean value (true or false)")
		}
	}
